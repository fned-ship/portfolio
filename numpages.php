...
$numberOfResults = $database->prepare("SELECT * FROM users ");
...
for($count = 1; $count<= $totalPages; ++$count){
    if($page == $count){
        echo '<a style="color:black;" href="numpages.php?page='.$count.'">'.$count.'</a> ';
    }else{
        echo '<a  href="numpages.php?page='.$count.'">'.$count.'</a> ';
    }
}
// 6) تحديد رقم البداية المحدد للنتائج في صفحة العرض
// determine the sql LIMIT starting number for the results on the displaying page

$results = $database->prepare("SELECT * FROM users LIMIT " . $resultsPerPage . " OFFSET " . ($page-1)*$resultsPerPage);
$results->execute();
...
foreach($results  AS $result){
    echo '<div class="shadow p-3 mb-3">'. $result['email'] . '</div>';
}
