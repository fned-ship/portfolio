...
<div id="nav-phone" >
    ...
    <svg class="svg-icon" width="30px" height="30px" viewBox="0 0 512 512" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
        <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
            <g id="work-case" fill="hsl(var(--primary-hsl))" transform="translate(42.666667, 64.000000)">
    ...
    <svg class="svg-icon" width="30px" height="30px" viewBox="0 0 24 24" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
        ...
        <g id="导航图标" stroke="var(--svg-icon-color)" stroke-width="1.5" transform="translate(-329.000000, -334.000000)">
...
<div class="full-screen-container">
    <div class="login-container"  style='margin-top:80px;'  >
        <h1 class="login-title" id="errorMsg" >Work History</h1>
        <h4 class='h4' style="margin-top:10px;" >if you work by yourself write 'work by myself' in employer section</h4>
        <form class="form" method="POST" >
            <div  id='job_container' style='width: 100%;' >
                ...
                <h4>job title</h4>
                <input type="text" class='req job-title' onchange="handleChange(this,1,0)" name="jobTitle"  required >
                </div>

                <div class="input-group" id="birthDiv" >
                <h4>Employer</h4>
                <input type="text" class='req employer'  onchange="handleChange(this,2,0)" name="employer"    required >
                </div>

                <div class="input-group" id="professionDiv" >
                <h4>city</h4>
                <input type="text" name="city"  onchange="handleChange(this,3,0)"  class='req city' required >
                </div>

                <div class="input-group" id="cityDiv" >
                <h4>country</h4>
                <input type="text" name="country" onchange="handleChange(this,4,0)"  class='req country' required >
                </div>

                <div class="input-group" id="cityDiv" >
                <h4>job description</h4>
                <input type="text" style="width:100%;" onchange="handleChange(this,5,0)"  required class='req job-desc' name="skill1" placeholder="e.g. Contributed ideas and suggestions in team meetings" onchange="handleChange(1)" >
                </div>

                <div class="input-group" id="countryDiv" >
                    <h4>start date</h4>
                    <input type="date" name="sDate"  onchange="handleChange(this,6,0)"  class='req start-date'  required >
                </div>
                <div class="input-group" id="countryDiv" >
                    <h4>end date</h4>
                    <div style='display:flex; flex-direction:row;' >
                        <input type="date" name="eDate"  onchange="handleChange(this,7,0)"  class='end-date' style='width:80%;'  >
                        <div style='display:flex;justify-content: center;flex-direction:column;align-items: center;gap:5px;width:20%;' >
                            <h4 style='color: hsl(var(--primary-light-hsl), 1);' >current</h4>
                            <input type='checkbox'  onchange="handleChange(this,8,0)"  class="current" >
                        </div>
                    </div>
                </div>
            ...
            <div  id='add_button' >add job experience</div>
...
function handleChange(input,section,skillNum){
    switch (section){
        case 1:
            data[skillNum].jobTitle=input.value ;
            break;
        case 2:
            data[skillNum].employer=input.value ;
            break;
        case 3:
            data[skillNum].city=input.value ;
            break
        case 4:
            data[skillNum].country=input.value ;
            break
        case 5:
            data[skillNum].jobDesc=input.value ;
            break 
        case 6:
            data[skillNum].startDate=input.value ;
            break ;
        case 8:
            data[skillNum].current=input.checked ;
            break;
        case 7:
            data[skillNum].endDate=input.value ;
            break
    }
...
function trashClick(skillNum){
    let job_title=document.querySelectorAll(".job-title");
        let employer=document.querySelectorAll(".employer");
        let city=document.querySelectorAll(".city");
        let country=document.querySelectorAll(".country");
        let start_date=document.querySelectorAll(".start-date");
        let job_desc=document.querySelectorAll(".job-desc");

        job_title[skillNum].required=false;
                employer[skillNum].required=false;
                city[skillNum].required=false
                country[skillNum].required=false
                job_desc[skillNum].required=false
                start_date[skillNum].required=false
...
add.addEventListener('click',event=>{
    ...
    job_container.innerHTML+=`
        <div class="job_container" style='width: 100%;display: flex;justify-content: center;flex-direction:column;align-items: center;gap:40px;'   >
             <div class="input-group" id="lNameDiv" style='margin-top:70px;' >
                <h4>job title</h4>
                <input type="text" class='req job-title' onchange="handleChange(this,1,${skillNum})" name="jobTitle"  required >
                </div>

                <div class="input-group" id="birthDiv" >
                <h4>Employer</h4>
                <input type="text" class='req employer'  onchange="handleChange(this,2,${skillNum})" name="employer"    required >
                </div>

                <div class="input-group" id="professionDiv" >
                <h4>city</h4>
                <input type="text" name="city"  onchange="handleChange(this,3,${skillNum})"  class='req city' required >
                </div>

                <div class="input-group" id="cityDiv" >
                <h4>country</h4>
                <input type="text" name="country" onchange="handleChange(this,4,${skillNum})"  class='req country' required >
                </div>

                <div class="input-group" id="cityDiv" >
                <h4>job description</h4>
                <input type="text" style="width:100%;" onchange="handleChange(this,5,${skillNum})"  required class='req job-desc' name="skill1" placeholder="e.g. Contributed ideas and suggestions in team meetings" onchange="handleChange(1)" >
                </div>

                <div class="input-group" id="countryDiv" >
                    <h4>start date</h4>
                    <input type="date" name="sDate"  onchange="handleChange(this,6,${skillNum})"  class='req start-date'  required >
                </div>
                <div class="input-group" id="countryDiv" >
                    <h4>end date</h4>
                    <div style='display:flex; flex-direction:row;' >
                        <input type="date" name="eDate"  onchange="handleChange(this,7,${skillNum})"  class='end-date' style='width:80%;'  >
                        <div style='display:flex;justify-content: center;flex-direction:column;align-items: center;gap:5px;width:20%;' >
                            <h4 style='color: hsl(var(--primary-light-hsl), 1);' >current</h4>
                            <input type='checkbox'  onchange="handleChange(this,8,${skillNum})"  class="current" >
                        </div>
                    </div>
                </div>
        ...
        let country=document.querySelectorAll(".country");
        let start_date=document.querySelectorAll(".start-date");
        let end_date=document.querySelectorAll(".end-date");
        let current=document.querySelectorAll(".current");
        let job_desc=document.querySelectorAll(".job-desc");
        for(let i=0;i<=skillNum;i++){
            try{
                job_title[i].value=data[i].jobTitle;
                employer[i].value=data[i].employer;
                city[i].value=data[i].city;
                country[i].value=data[i].country;
                job_desc[i].value=data[i].jobDesc;
                start_date[i].value=data[i].startDate;
                if(data[i].current){
                    current[i].checked=true;
                }else{
                    end_date[i].value=data[i].endDate;
                }
            }catch (error) {
...
if(isset($_SESSION['user'])){
    if($_SESSION['user']->role === "user"){
        ...
        $getWork = $database->prepare("SELECT * FROM cv7  WHERE cv7_id=:cv_id ");
        $getWork->bindParam("cv_id",$_SESSION['user']->id);
        $getWork->execute();

        if($getWork->rowCount()>0){
            $Work=$getWork->fetchObject();
            $Work_data=json_decode($Work->job_data);
            ...
            if($Work_data!=''){
                ...
                data[0].jobTitle="'.$Work_data[0]->jobTitle.'" ;
                data[0].employer="'.$Work_data[0]->employer.'";
                data[0].city="'.$Work_data[0]->city.'" ;
                data[0].country="'.$Work_data[0]->country.'" ;
                data[0].jobDesc="'.$Work_data[0]->jobDesc.'" ;
                data[0].startDate="'.$Work_data[0]->startDate.'";
                if('.currentVal($Work_data[0]->current).' ==200){
                    data[0].current=true;
                }else{
                    data[0].endDate="'.$Work_data[0]->endDate.'";
                }
    
                job_title[0].value=data[0].jobTitle;
                employer[0].value=data[0].employer;
                city[0].value=data[0].city;
                country[0].value=data[0].country;
                job_desc[0].value=data[0].jobDesc;
                start_date[0].value=data[0].startDate;
                if(data[0].current){
                    current[0].checked=true;
                }else{
                    end_date[0].value=data[0].endDate;
                }
                ...
                for ($i=1;$i<count($Work_data);$i++){
                    ...
                    job_container.innerHTML+=`
                        <div class='job_container' style='width: 100%;display: flex;justify-content: center;flex-direction:column;align-items: center;gap:40px;'   >
                             <div class='input-group' id='lNameDiv' style='margin-top:70px;' >
                                <h4>job title</h4>
                                <input type='text' class='req job-title' onchange='handleChange(this,1,".$i.")' name='jobTitle'  required >
                                </div>
                
                                <div class='input-group' id='birthDiv' >
                                <h4>Employer</h4>
                                <input type='text' class='req employer'  onchange='handleChange(this,2,".$i.")'     required >
                                </div>
                
                                <div class='input-group' id='professionDiv' >
                                <h4>city</h4>
                                <input type='text'   onchange='handleChange(this,3,".$i.")'  class='req city' required >
                                </div>
                
                                <div class='input-group' id='cityDiv' >
                                <h4>country</h4>
                                <input type='text'  onchange='handleChange(this,4,".$i.")'  class='req country' required >
                                </div>
                
                                <div class='input-group' id='cityDiv' >
                                <h4>job description</h4>
                                <input type='text' style='width:100%;' onchange='handleChange(this,5,".$i.")'  required class='req job-desc' name='skill1' placeholder='e.g. Contributed ideas and suggestions in team meetings' onchange='handleChange(1)' >
                                </div>
                
                                <div class='input-group' id='countryDiv' >
                                    <h4>start date</h4>
                                    <input type='date' name='sDate'  onchange='handleChange(this,6,".$i.")'  class='req start-date'  required >
                                </div>
                                <div class='input-group' id='countryDiv' >
                                    <h4>end date</h4>
                                    <div style='display:flex; flex-direction:row;' >
                                        <input type='date' name='eDate'  onchange='handleChange(this,7,".$i.")'  class='end-date' style='width:80%;'  >
                                        <div style='display:flex;justify-content: center;flex-direction:column;align-items: center;gap:5px;width:20%;' >
                                            <h4 style='color: hsl(var(--primary-light-hsl), 1);' >current</h4>
                                            <input type='checkbox'  onchange='handleChange(this,8,".$i.")'  class='current' >
                                        </div>
                                    </div>
                                </div>
                                <svg xmlns='http://www.w3.org/2000/svg' style='background:rgba(255,255,255,0.5); border-radius:3px;'  onclick='trashClick(".$i.")' class='trash' width='24' height='24' style='fill: rgb(59, 19, 19);'><path d='M6 7H5v13a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V7H6zm4 12H8v-9h2v9zm6 0h-2v-9h2v9zm.618-15L15 2H9L7.382 4H3v2h18V4z'></path></svg>
                
                            </div>
                
                         `;
    
                         data[skillNum].jobTitle='".$Work_data[$i]->jobTitle."' ;
                         data[skillNum].employer='".$Work_data[$i]->employer."';
                         data[skillNum].city='".$Work_data[$i]->city."' ;
                         data[skillNum].country='".$Work_data[$i]->country. "'
                         data[skillNum].jobDesc='".$Work_data[$i]->jobDesc."' ;
                         data[skillNum].startDate='".$Work_data[$i]->startDate."';
                         if( ".currentVal($Work_data[$i]->current)." ==200){
                             data[skillNum].current=true;
                         }else{
                             data[skillNum].endDate='".$Work_data[$i]->endDate."';
                         }
                        ...
                        job_title=document.querySelectorAll('.job-title');
                        employer=document.querySelectorAll('.employer');
                        city=document.querySelectorAll('.city');
                        country=document.querySelectorAll('.country');
                        start_date=document.querySelectorAll('.start-date');
                        end_date=document.querySelectorAll('.end-date');
                        current=document.querySelectorAll('.current');
                        job_desc=document.querySelectorAll('.job-desc');
        ...
        if(isset($_POST['send'])){
            if($getWork->rowCount()==0){
                $send = $database->prepare("INSERT INTO cv7 (job_data,cv7_id) VALUES(:job_data,:cv7_id)");
                $send->bindParam("job_data",$_POST['data']);
                $send->bindParam("cv7_id",$_SESSION['user']->id);
                $send->execute();
            }else{
                $send = $database->prepare("UPDATE  cv7 SET  job_data=:job_data WHERE cv7_id=:cv7_id ");
                $send->bindParam("job_data",$_POST['data']);
                $send->bindParam("cv7_id",$_SESSION['user']->id);
                $send->execute();
            }
            //header('Location: cv8.php' , true);
            echo "<script> location.replace('certification.php') </script>";
        }
    }else{
