...
<div id="nav-phone" >
    ...
    <svg class="svg-icon" fill="hsl(var(--primary-hsl))" height="30px" width="30px" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"  viewBox="0 0 431.17 431.17" xml:space="preserve">
        <g>
...
<div class="full-screen-container">
   <div class="login-container"  style='margin-top:80px;'  >
      <h1 class="login-title" id="errorMsg" >Certifications</h1>
      <form class="form" method="POST" >
        <div id='formDiv' >
          <div class="input-group" id="SKILL1" >
            <div>
                <p>certificate name</p>
                <input type="text" class="input" required name="certName" id="skill11" onchange="handleChange(1,1)"  >
            </div>
            <div>
                <p>date</p>
                <input type="date" class="input" required name="certDate" id="skill12" onchange="handleChange(1,2)"  >
            </div>
            <div>
                <p>academy name</p>
                <input type="text" class="input" required  name="academyName" id="skill13" onchange="handleChange(1,3)"  >
            </div>
            <div class="checkbox-div" >
                <div>
                    <input type="checkbox" class="typeline1" onclick="onlyOne(this,1)" name="online" id="skill14" onchange="handleChange(1,4)"  >
                    <p> online courses </p>
                </div>
                <div>
                    <input type="checkbox"  class="typeline1" onclick="onlyOne(this,1)" name="offline" id="skill15" onchange="handleChange(1,5)"  >
                    <p> offline courses </p>
                </div>
            </div>
            <div class="rating" >
                <svg xmlns="http://www.w3.org/2000/svg" id='trash1' onclick='trashClick(1)' class='trash' width="30" height="30" style="fill: rgb(59, 19, 19);"><path d="M6 7H5v13a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V7H6zm4 12H8v-9h2v9zm6 0h-2v-9h2v9zm.618-15L15 2H9L7.382 4H3v2h18V4z"></path></svg>
            </div>
          </div>
        </div>
        <input type="hidden" name='data' id='hiddenInput' >
        <div  id='add_button' >add one</div>
        <button type="submit" id='send' name="send" class="send-button">next</button>
...
var data={
    certificate:[{certName:'',certDate:'',academyName:'',online:false,offline:false}]
}


function onlyOne(checkbox,skillNum) {
    var checkboxes = document.querySelectorAll(`.typeline${skillNum}`)
    checkboxes.forEach((item) => {
        //console.log(item === checkbox)
        let key =item.name;
        if (item !== checkbox) {
            item.checked = false
            data.certificate[skillNum-1][key]=false
        }else{
            data.certificate[skillNum-1][key]=true
        }
    })
...
// function onlyOne(range,skillNum) {
//     if(range===1){
//         data.certificate[skillNum-1].online=true
//         data.certificate[skillNum-1].offline=false
//         let ID='skill'+String(skillNum)+"5"
//         let offcheck=document.getElementById(ID);
//         offcheck.checked=false
//     }else{
//         data.certificate[skillNum-1].online=false
//         data.certificate[skillNum-1].offline=true
//         let ID='skill'+String(skillNum)+"4"
//         let oncheck=document.getElementById(ID);
...
function handleChange(skillNum,range){
    var ID='skill'+String(skillNum)+String(range);
    var INPUT=document.getElementById(ID);
    let key=INPUT.name;
    if(range<4){
        let value=INPUT.value
        data.certificate[skillNum-1][key]=value
    }
...
function trashClick(skillNum){
    ...
    let i=skillNum ;
    for(let j=1 ;j<6;j++){
            var ID='skill'+String(i)+String(j);
    ...
    var ID='SKILL'+String(skillNum)
    var SKILL=document.getElementById(ID);
    SKILL.style.display='none'
    data.certificate.splice(skillNum-1,1)
...
add.addEventListener('click',event=>{
    data.certificate.push({certName:'',certDate:'',academyName:'',online:false,offline:false});
    skillNum+=1;
    form=document.getElementById('formDiv');
    form.innerHTML+=`
    <div class="input-group" id="SKILL${skillNum}" >
            <div>
                <p>certificate name</p>
                <input type="text"  class="input" required name="certName" id="skill${skillNum}1" onchange="handleChange(${skillNum},1)"  >
            </div>
            <div>
                <p>date</p>
                <input type="date" class="input" required name="certDate" id="skill${skillNum}2" onchange="handleChange(${skillNum},2)"  >
            </div>
            <div>
                <p>academy name</p>
                <input type="text" class="input" required  name="academyName" id="skill${skillNum}3" onchange="handleChange(${skillNum},3)"  >
            </div>
            <div class="checkbox-div" >
                <div>
                    <input type="checkbox"  class="typeline${skillNum}" onclick="onlyOne(this,${skillNum})" name="online" id="skill${skillNum}4" onchange="handleChange(${skillNum},4)"  >
                    <p> online courses </p>
                </div>
                <div>
                    <input type="checkbox"  class="typeline${skillNum}" onclick="onlyOne(this,${skillNum})" name="offline" id="skill${skillNum}5" onchange="handleChange(${skillNum},5)"  >
                    <p> offline courses </p>
                </div>
            </div>
            <div class="rating" >
                <svg xmlns="http://www.w3.org/2000/svg" id='trash1' onclick='trashClick(${skillNum})' class='trash' width="30" height="30" style="fill: rgb(59, 19, 19);"><path d="M6 7H5v13a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V7H6zm4 12H8v-9h2v9zm6 0h-2v-9h2v9zm.618-15L15 2H9L7.382 4H3v2h18V4z"></path></svg>
            </div>
          </div>
    `;
    for(var i=1;i<skillNum;i++){
        for(let j=1 ;j<6;j++){
            ...
            var key=skill.name ;
            if(j<4){
                skill.value=data.certificate[i-1][key]
            }else{
                //console.log("key:",key," is checked: ",data.certificate[i-1][key])
                if(data.certificate[i-1][key]){
                    skill.checked=true
                }else{
                    skill.checked=false
                }
...
if(isset($_SESSION['user'])){
    if($_SESSION['user']->role === "user"){
        $getCert = $database->prepare("SELECT * FROM cv11  WHERE cv11_id=:cv_id ");
        $getCert->bindParam("cv_id",$_SESSION['user']->id);
        $getCert->execute();

        if($getCert->rowCount()>0){
            $Cert=$getCert->fetchObject();
            $Cert_data=json_decode($Cert->certJSON);

            if($Cert_data!=""){
                echo '<script>

            data.certificate[0].certName="'.$Cert_data->certificate[0]->certName.'" ;
            data.certificate[0].certDate="'.$Cert_data->certificate[0]->certDate.'";
            data.certificate[0].academyName="'.$Cert_data->certificate[0]->academyName.'" ;
            data.certificate[0].online="'.$Cert_data->certificate[0]->online.'" ;
            data.certificate[0].offline="'.$Cert_data->certificate[0]->offline.'" ;
            ...
            for ($i=1;$i<count($Cert_data->certificate);$i++){
                echo '<script>
                data.certificate.push({certName:"",certDate:"",academyName:"",online:false,offline:false});
                skillNum+=1;
                form=document.getElementById("formDiv");
                ...
                data.certificate[skillNum-1].certName="'.$Cert_data->certificate[$i]->certName.'" ;
                data.certificate[skillNum-1].certDate="'.$Cert_data->certificate[$i]->certDate.'";
                data.certificate[skillNum-1].academyName="'.$Cert_data->certificate[$i]->academyName.'" ;
                data.certificate[skillNum-1].online="'.$Cert_data->certificate[$i]->online.'" ;
                data.certificate[skillNum-1].offline="'.$Cert_data->certificate[$i]->offline.'" ;
        ...
        if(isset($_POST['send'])){
            if($getCert->rowCount()==0){
                $send = $database->prepare("INSERT INTO cv11 (certJSON,cv11_id) VALUES(:certJSON,:cv11_id)");
                $send->bindParam("certJSON",$_POST['data']);
                $send->bindParam("cv11_id",$_SESSION['user']->id);
                $send->execute();
            }else{
                $send = $database->prepare("UPDATE cv11 SET certJSON=:certJSON WHERE cv11_id=:cv11_id");
                $send->bindParam("certJSON",$_POST['data']);
                $send->bindParam("cv11_id",$_SESSION['user']->id);
                $send->execute();
            }
            //header('Location: cv3.php' , true);
            echo "<script> location.replace('language.php') </script>";
        }
    }else{
