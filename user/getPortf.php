...
function cryptage($val){
    $ch='';
    $strVal=strval($val);
    $arr=['bN','Fq','qq','To','dL','Rg','DD','bO','Rs','mm'];
    for ($i=0;$i<strlen($strVal);$i++){
        $ch+=$arr[intval($strVal[$i])];
    }
...
if(isset($_SESSION['user'])){
    if($_SESSION['user']->role === "user"){
        ...
        $getResumeHeading->bindParam("id",$_SESSION['user']->id);
        $getResumeHeading->execute();

        $getSummary = $database->prepare("SELECT * FROM cv6  WHERE cv6_id=:id ");
        $getSummary->bindParam("id",$_SESSION['user']->id);
        $getSummary->execute();

        $getLanguage = $database->prepare("SELECT * FROM cv5  WHERE cv5_id=:id ");
        $getLanguage->bindParam("id",$_SESSION['user']->id);
        $getLanguage->execute();

        $getContact = $database->prepare("SELECT * FROM cv2  WHERE cv1_id=:id ");
        $getContact->bindParam("id",$_SESSION['user']->id);
        $getContact->execute();

        $getColor = $database->prepare("SELECT * FROM color  WHERE user_id=:id ");
        $getColor->bindParam("id",$_SESSION['user']->id);
        $getColor->execute();

        if($getResumeHeading->rowCount()==0){
            header('Location: portfHeading.php' , true);
        }elseif($getSummary->rowCount()==0){
            header('Location: summary.php' , true);
        }elseif($getLanguage->rowCount()==0){
            header('Location: language.php' , true);
        }elseif($getContact->rowCount()==0){
            header('Location: contact.php' , true);
        }elseif($getColor->rowCount()==0){
            header('Location: color.php' , true);
        }else{
            $getRow = $database->prepare("SELECT * FROM users  WHERE id=:id ");
            $getRow->bindParam("id",$_SESSION['user']->id);
            $getRow->execute();

            $obj=$getRow->fetchObject() ;

            $url=$webURL."portfolio.php?token=".$obj->firstName.".".$obj->lastName.'.'.$_SESSION['user']->id;
        }
    }else{
...
<div class="full-screen-container">
   <div class="login-container" style="position:relative;" >
      <div class="form" method="POST"  >
            <h2 style="text-align:center ; margin:0; color:green;" >your potfolio website has been successfully created !</h2>
            <div style='display:flex;justify-content:center;align-items:center;gap:5px;flex-wrap:wrap; ' >  <!-- flex-wrap:wrap; -->
                <p style='color:white;border: 1px solid hsl(var(--primary-hsl));word-break:break-all;margin:0;font-size:1.2em; background-color: hsl(var(--primary-hsl), .25); padding: .5em 1em; border-radius: .25em 0 0 .25em ; ' ><?php echo $url ; ?></p>
                <button id="copyButton" class='copy-button'  data-copy-text="<?php echo $url ; ?>">Copy URL</button>
            </div>
            <button type="submit"  name="submit" onclick="visitWeb()" class="send-button">visit potfolio website</button>
        </div>
   </div>
</div>
<script>
