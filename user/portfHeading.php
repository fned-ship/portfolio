...
<div class="full-screen-container">
   <div class="login-container"  style='margin-top:80px;'  >
      <h1 class="login-title" id="errorMsg" >Resume Heading</h1>
      <form class="form" method="POST"  enctype="multipart/form-data" >
        ...
        <div class="input-group" id="birthDiv" >
         ...
         <span class="msg" id="birthMsg" >try to write it correctly</span>
        </div>
        ...
        <div class='input-file' >
            <label for="inputTag">
                ...
                <input id="inputTag" name='picture' required accept="image/*" type="file"/>
                <br/>
        ...
        <div class='input-file' >
            <label for="vidImagTag">
                Select background image<br/>
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" style="fill: rgba(0, 0, 0, 1);"><path d="M12 9c-1.626 0-3 1.374-3 3s1.374 3 3 3 3-1.374 3-3-1.374-3-3-3z"></path><path d="M20 5h-2.586l-2.707-2.707A.996.996 0 0 0 14 2h-4a.996.996 0 0 0-.707.293L6.586 5H4c-1.103 0-2 .897-2 2v11c0 1.103.897 2 2 2h16c1.103 0 2-.897 2-2V7c0-1.103-.897-2-2-2zm-8 12c-2.71 0-5-2.29-5-5s2.29-5 5-5 5 2.29 5 5-2.29 5-5 5z"></path></svg>
                <input id="vidImagTag" style='display:none;' name='imVid' required accept="image/*,video/*" type="file"/>
                <br/>
                <span id="imageVidName"  style='color:green;' ></span>
            </label>
        </div>

        <button type="submit" name="send" class="send-button">next</button>
      </form>
   </div>
</div>
<script>
    ...
    let vidImagTag = document.getElementById("vidImagTag");
    let imageVidName = document.getElementById("imageVidName");

         vidImagTag.addEventListener("change", ()=>{
            // let inputImage = document.querySelector("input[type=file]").files[0];
            imageVidName.innerText = vidImagTag.files[0].name;
        })
</script>
...
if(isset($_SESSION['user'])){
    if($_SESSION['user']->role === "user"){
        $checkRow = $database->prepare("SELECT * FROM cv WHERE cv_id = :cv_id");
        $checkRow->bindParam("cv_id",$_SESSION['user']->id);
        $checkRow->execute();

        $getPic = $database->prepare("SELECT * FROM profilepicture  WHERE pic_id=:cv_id ");
        $getPic->bindParam("cv_id",$_SESSION['user']->id);
        $getPic->execute();

        $getBg = $database->prepare("SELECT * FROM img_vid  WHERE user_id=:cv_id ");
        $getBg->bindParam("cv_id",$_SESSION['user']->id);
        $getBg->execute();

        if($checkRow->rowCount()>0){
            $row=$checkRow->fetchObject();

            $Pic = $getPic->fetchObject();
            $fileName=$Pic->name;

            $Bg = $getBg->fetchObject();
            $fileName1=$Bg->name;
        ...
        if(isset($_POST['send'])){
            ...
            if($getBg->rowCount()==0){
                $fileType = $_FILES["imVid"]["type"];
                $fileSize = $_FILES["imVid"]["size"]/1024**2; //mega
                $fileName = $_FILES["imVid"]["name"];
                $tmpName = $_FILES["imVid"]["tmp_name"] ;
                $file =file_get_contents($tmpName);

                $uploadFile = $database->prepare("INSERT INTO img_vid (name,size,user_id,type,file) VALUES(:name,:size,:user_id,:type,:file)");
                ...
                $uploadFile->bindParam("user_id",$_SESSION['user']->id);
                $uploadFile->execute();
            }elseif($getBg->rowCount()>0 && $_FILES["imVid"]["size"]!=0){
                ...
                $uploadFile = $database->prepare("UPDATE img_vid SET name = :name , size = :size , type = :type, file = :file  WHERE user_id=:user_id ;");
                $uploadFile->bindParam("name",$fileName);
                $uploadFile->bindParam("type",$fileType);
                $uploadFile->bindParam("file",$file);
                $uploadFile->bindParam("size",$fileSize);
            ...
            if($getPic->rowCount()==0){
                $fileType = $_FILES["picture"]["type"];
                $fileSize = $_FILES["picture"]["size"]/1024**2; //mega
                $fileName = $_FILES["picture"]["name"];
                $file =file_get_contents($_FILES["picture"]["tmp_name"]);

                $uploadFile = $database->prepare("INSERT INTO profilepicture(file,name,type,size,pic_id) VALUES(:file,:name,:type,:size,:pic_id)");
                $uploadFile->bindParam("name",$fileName);
                $uploadFile->bindParam("type",$fileType);
                $uploadFile->bindParam("file",$file);
                $uploadFile->bindParam("size",$fileSize);
                $uploadFile->bindParam("pic_id",$_SESSION['user']->id);
                $uploadFile->execute();
            }elseif($getPic->rowCount()>0 && $_FILES["picture"]["size"]!=0){
                $fileType = $_FILES["picture"]["type"];
                $fileSize = $_FILES["picture"]["size"]/1024**2; //mega
                $fileName = $_FILES["picture"]["name"];
                $file =file_get_contents($_FILES["picture"]["tmp_name"]);

                $uploadFile = $database->prepare("UPDATE profilepicture SET name = :name , size = :size , type = :type, file = :file  WHERE pic_id=:pic_id ;");
                $uploadFile->bindParam("name",$fileName);
                $uploadFile->bindParam("type",$fileType);
                $uploadFile->bindParam("file",$file);
                $uploadFile->bindParam("size",$fileSize);
            ...
            if($checkRow->rowCount()==0){
                $token=time().$_SESSION['user']->id;

                $send = $database->prepare("INSERT INTO cv (firstName,lastName,birth,profession,country,cv_id,token) VALUES(:fname,:lname,:birth,:profession,:country,:cv_id,:token)");
                $send->bindParam("fname",$_POST['fName']);
                $send->bindParam("lname",$_POST['lName']);
                $send->bindParam("profession",$_POST['profession']);
                $send->bindParam("country",$_POST['country']);
                $send->bindParam("birth",$_POST['birth']);
                $send->bindParam("cv_id",$_SESSION['user']->id);
                $send->bindParam("token",$token);
                $send->execute();
            }else{
                $send = $database->prepare("UPDATE cv SET firstName = :fname , lastName = :lname , birth = :birth, profession = :profession , country = :country   WHERE cv_id=:cv_id ;)");
                $send->bindParam("fname",$_POST['fName']);
                $send->bindParam("lname",$_POST['lName']);
                $send->bindParam("profession",$_POST['profession']);
                $send->bindParam("country",$_POST['country']);
            ...
            // header('Location: summary.php' , true);
           echo "<script> location.replace('summary.php') </script>";
        }
    }else{
