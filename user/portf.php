...
<body>
    ...
    <svg class="svg-icon" fill="hsl(var(--primary-hsl))" height="30px" width="30px" version="1.2" baseProfile="tiny" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
...
<div class="full-screen-container">
    <div class="login-container" style='margin-top:80px;' >
        <h1 class="login-title" id="errorMsg" >Portfolio</h1>
        <form class="form" method="POST" enctype="multipart/form-data" >
            <div  id='job_container' style='width: 100%;' >
                ...
                <h4>Project Title</h4>
                <input type="text" class='req project-title' onchange="handleChange(this,1,0)" name="projectTitle"  required >
                </div>

                <div class="input-group" id="birthDiv" >
                <h4>Project description</h4>
                <input type="text" class='req project-desc'  onchange="handleChange(this,2,0)" name="projectTitle'"    required >
                </div>

                <div class="input-group" id="birthDiv" >
                <h4>Project Link</h4>
                <input type="text" class='req project-link'  onchange="handleChange(this,3,0)" name="projectLink'"    placeholder='not required'  >
                </div>

                <div class='input-file' >
                    <label for="inputTag0">
                        Select a project image <br/>
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" style="fill: rgba(0, 0, 0, 1);"><path d="M12 9c-1.626 0-3 1.374-3 3s1.374 3 3 3 3-1.374 3-3-1.374-3-3-3z"></path><path d="M20 5h-2.586l-2.707-2.707A.996.996 0 0 0 14 2h-4a.996.996 0 0 0-.707.293L6.586 5H4c-1.103 0-2 .897-2 2v11c0 1.103.897 2 2 2h16c1.103 0 2-.897 2-2V7c0-1.103-.897-2-2-2zm-8 12c-2.71 0-5-2.29-5-5s2.29-5 5-5 5 2.29 5 5-2.29 5-5 5z"></path></svg>
                        <input class='req'  id='inputTag0' onchange="inputHandleChange(this,0)" style='display:none;' name='picture0' required accept="image/*" type="file"/>
                        <br/>
            ...
            <div  id='add_button' >add project</div>
...
<script>
    ...
    function getBase64(file,num,ImgName,imgFullName,input) {
        let reader = new FileReader();

        reader.readAsDataURL(file);
        reader.onload = function () {
            data[num].base64.val=reader.result ;
            data[num].base64.success=true;

            data[num].imageName=imgFullName;

            myJSON = JSON.stringify(data);
            hidInput.value=myJSON;

            let imageName = document.querySelectorAll(".imageName");
            imageName[num].innerText = ImgName ;
        ...
        reader.onerror = function (error) {
            data[num].base64.val=error;
            data[num].base64.success=false;

            myJSON = JSON.stringify(data);
            hidInput.value=myJSON;

            let imageName = document.querySelectorAll(".imageName");
            imageName[num].innerText = error ;
        };
    ...
    let inputHandleChange=(input,num)=>{
        let imageName = document.querySelectorAll(".imageName");
        let inputImage = input.files[0];

        imageName[num].innerText = 'waiting ...';

        let d = new Date();
        let time = d.getTime();
        let randInt=Math.round(Math.random()*10000);
        let userId=<?php echo $_SESSION['user']->id  ?>;


        getBase64(inputImage,num, inputImage.name,`${time}${randInt}${userId}${inputImage.name}`,input)
        // console.log(data[num].imageName);
...
function handleChange(input,section,skillNum){
    switch (section){
        case 1:
            data[skillNum].projectTitle=input.value ;
            break;
        case 2:
            data[skillNum].projectDesc=input.value ;
            break;
        case 3:
            data[skillNum].projectLink=input.value ;
            break;
    }
...
function trashClick(skillNum){
    let project_title=document.querySelectorAll(".project-title");
    let project_desc=document.querySelectorAll(".project-desc");
    let inputTag=document.getElementById('inputTag'+String(skillNum));

    project_title[skillNum].required=false
    project_desc[skillNum].required=false
    inputTag.required=false
...
add.addEventListener('click',event=>{
    ...
    <div class="job_container" style='width: 100%;display: flex;justify-content: center;flex-direction:column;align-items: center;gap:40px;'  >
                <div class="input-group" id="lNameDiv" >
                <h4>project title</h4>
                <input type="text" class='req project-title' onchange="handleChange(this,1,${skillNum})" name="projectTitle"  required >
                </div>

                <div class="input-group" id="birthDiv" >
                <h4>project description</h4>
                <input type="text" class='req project-desc'  onchange="handleChange(this,2,${skillNum})" name="projectTitle"    required >
                </div>

                <div class="input-group" id="birthDiv" >
                <h4>Project Link</h4>
                <input type="text" class='req project-link'  onchange="handleChange(this,3,${skillNum})" name="projectLink" placeholder='not required'  >
                </div>

                <div class='input-file' >
                    <label for="inputTag${skillNum}">
                        Select a project image <br/>
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" style="fill: rgba(0, 0, 0, 1);"><path d="M12 9c-1.626 0-3 1.374-3 3s1.374 3 3 3 3-1.374 3-3-1.374-3-3-3z"></path><path d="M20 5h-2.586l-2.707-2.707A.996.996 0 0 0 14 2h-4a.996.996 0 0 0-.707.293L6.586 5H4c-1.103 0-2 .897-2 2v11c0 1.103.897 2 2 2h16c1.103 0 2-.897 2-2V7c0-1.103-.897-2-2-2zm-8 12c-2.71 0-5-2.29-5-5s2.29-5 5-5 5 2.29 5 5-2.29 5-5 5z"></path></svg>
                        <input  id='inputTag${skillNum}' class='req' onchange="inputHandleChange(this,${skillNum})" style='display:none;' name='picture${skillNum}' required accept="image/*" type="file"/>
                        <br/>
                        <span class="imageName" ></span>
                    </label>
                </div>
    ...
    for(let i=0;i<=skillNum;i++){
        let project_title=document.querySelectorAll(".project-title");
        let project_desc=document.querySelectorAll(".project-desc");
        let project_link=document.querySelectorAll(".project-link");
        for(let i=0;i<=skillNum;i++){
            try{
                project_title[i].value=data[i].projectTitle;
                project_desc[i].value=data[i].projectDesc;
                project_link[i].value=data[i].projectLink;
            }catch (error) {
...
if(isset($_SESSION['user'])){
    if($_SESSION['user']->role === "user"){
        ...
        $getPortf = $database->prepare("SELECT * FROM projects  WHERE user_id=:cv_id ");
        $getPortf->bindParam("cv_id",$_SESSION['user']->id);
        $getPortf->execute();

        if($getPortf->rowCount()>0){
            $row=$getPortf->fetchObject();
            $portf_data=json_decode($row->projects_data);
            if($portf_data!=""){
                echo "<script>
                let project_title=document.querySelectorAll('.project-title');
                let project_desc=document.querySelectorAll('.project-desc');
                ...
                data[0].projectTitle='".$portf_data[0]->projectTitle."';
                data[0].projectDesc='".$portf_data[0]->projectDesc."';
                data[0].projectLink='".$portf_data[0]->projectLink."';
                data[0].imageName='".$portf_data[0]->imageName."';

                myJSON = JSON.stringify(data);
                hidInput.value=myJSON;

                project_title[0].value=data[0].projectTitle;
                project_desc[0].value=data[0].projectDesc;
                project_link[0].value=data[0].projectLink;
                imageName[0].innerHTML=data[0].imageName;

                document.getElementById('inputTag0').required = false ;

                </script>";

                for ($i=1;$i<count($portf_data);$i++){
                    echo '<script>
                    data.push({
                        projectTitle:"'.$portf_data[$i]->projectTitle.'",
                        projectDesc:"'.$portf_data[$i]->projectDesc.'",
                        projectLink:"'.$portf_data[$i]->projectLink.'",
                        imageName:"'.$portf_data[$i]->imageName.'",
                        base64:{
                            val: ""
                            ,success : null
                        }
                    ...
                    <div class="job_container" style="width: 100%;display: flex;justify-content: center;flex-direction:column;align-items: center;gap:40px;"  >
                                ...
                                <div class="input-file" >
                                    <label for="inputTag${skillNum}">
                                        Select a project image <br/>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" style="fill: rgba(0, 0, 0, 1);"><path d="M12 9c-1.626 0-3 1.374-3 3s1.374 3 3 3 3-1.374 3-3-1.374-3-3-3z"></path><path d="M20 5h-2.586l-2.707-2.707A.996.996 0 0 0 14 2h-4a.996.996 0 0 0-.707.293L6.586 5H4c-1.103 0-2 .897-2 2v11c0 1.103.897 2 2 2h16c1.103 0 2-.897 2-2V7c0-1.103-.897-2-2-2zm-8 12c-2.71 0-5-2.29-5-5s2.29-5 5-5 5 2.29 5 5-2.29 5-5 5z"></path></svg>
                                        <input class="req"  id="inputTag${skillNum}" onchange="inputHandleChange(this,${skillNum})" style="display:none;" name="picture${skillNum}" accept="image/*" type="file"/>
                                        <br/>
                                        <span class="imageName" >${data[skillNum].imageName}</span>
                                    </label>
                                </div>
        ...
        if(isset($_POST['send'])){
            ...
            if($data!=""){
                for($i=0;$i<count($data);$i++){
                        ...
                        // move_uploaded_file ($data[$i]->tmpName, "usersImg/".$data[$i]->imageName)

                        if($data[$i]->base64->success){
                            $base64Data = $data[$i]->base64->val ; // Replace this with your actual base64 data

                            $dataIm = substr($base64Data, strpos($base64Data, ',') + 1);

                            // Decode the base64 data into binary format
                            $fileData = base64_decode($dataIm);

                            // Specify the destination file path
                            $destination = "usersImg/".$data[$i]->imageName; // Replace this with your desired file path
           ...
           if($getPortf->rowCount()==0){
                $send = $database->prepare("INSERT INTO projects  (projects_data,user_id) VALUES(:projects_data,:user_id)");
                $send->bindParam("projects_data",$_POST['data']);
                $send->bindParam("user_id",$_SESSION['user']->id);
                $send->execute();
           }else{
                $send = $database->prepare("UPDATE projects SET projects_data=:projects_data WHERE user_id=:user_id");
                $send->bindParam("projects_data",$_POST['data']);
                $send->bindParam("user_id",$_SESSION['user']->id);
                $send->execute();
           }
